...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Los cambios se realizaron con exito')
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            flash('Usario {} no encontrado.'.format(username))
            return redirect(url_for('index'))
        if user == current_user:
            flash('No puedes seguirte a ti mismo!')
            return redirect(url_for('Usuario', username=username))
        current_user.follow(user)
        db.session.commit()
        flash('Estas siguiendo a {}!'.format(username))
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('No esta siguiendo a{}.'.format(username))
        return redirect(url_for('user', username=username))
    else:
        return redirect(url_for('index'))